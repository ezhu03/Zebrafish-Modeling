...
def update_velocities(positions, velocities, radius, speed, noise):
    ...
    th = np.random.uniform(0, 2*np.pi, num_agents)  # Generate n random angles
    unit_vectors = np.column_stack((np.cos(th), np.sin(th)))  # Convert to 2D vectors
...
for a in range(iterations):
    ...
    angles = np.random.uniform(0, 2*np.pi, num_agents)
    velocities = np.column_stack((np.cos(angles), np.sin(angles))) * spd    
    print(a, "...running...")
    ...
    for i in range(time):
       ...
       #print(weight)
        ...
        for j in range(num_agents):
            if newpositions[j][0]**2+newpositions[j][1]**2>(box_radius-0.2)**2:
...
# Plot the trajectory
        ...
        plt.xlim(-5, 5)
        plt.ylim(-5, 5)
        plt.savefig('/Users/ezhu/Downloads/vicsekmodel_circle_half_%s.png'%day, dpi=3000, bbox_inches='tight')
        plt.show()
center = (0, 0)
