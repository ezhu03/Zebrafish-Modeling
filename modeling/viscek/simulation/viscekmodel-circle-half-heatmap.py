...
def update_velocities(positions, velocities, radius, speed, noise):
    ...
    for i in range(num_agents):
        ...
        #print(current_state)
        speed[i] = np.abs(1.05+0.583*np.random.normal())
        noise[i] = 0.25*np.random.normal()
...
file_name = 'const%sradius%sboxradius%siter%sfish%s_15min.npz'%(const,radius,box_radius,iterations,num_agents)
