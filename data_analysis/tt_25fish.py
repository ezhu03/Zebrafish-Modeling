...
s = np.delete(tr.s, [3,6,21], axis = 1)
v= np.delete(tr.v, [3,6,21], axis =1)
a = np.delete(tr.a, [3,6,21], axis = 1)

positions = np.reshape(s, (s.shape[0]*s.shape[1] , 2))
velocities = np.reshape(v, (v.shape[0]*v.shape[1] , 2))
acc = np.reshape(a, (a.shape[0]*a.shape[1] , 2))
...
pos.rename(columns={0: 'x', 1: 'y'})
pos['center'] = np.sqrt(pos[0]**2 + pos[1]**2)
print(positions)
sns.histplot(data = pos, x=0, y=1, stat="density", bins = [20,20], common_norm=True, palette = sns.color_palette("RdBu", 10))
plt.show()
plt.hist2d(positions[:, 0], positions[: , 1], bins=(20, 20), cmap="hot", density=True, vmin = 0, vmax = 0.025)
plt.xlabel('X-bins')
plt.ylabel('Y-bins')
plt.title('Heatmap for 25 Fish Clear Tank')
plt.colorbar(label='Frequency')
plt.show()
sns.histplot(data = pos, x='center', bins = 40, binrange = [0,16])
plt.show()
