...
plt.show()
"""
This script loads single-fish idtracker.ai trajectories for **half-sanded arenas** (one side rough, one side clear),
optionally for blind (TYR) fish. It converts positions/velocities to physical units, then produces:
  • Arena-centered position heatmaps
  • Distributions of r, speed, radial speed, and near‑wall angle φ
  • Side‑split (x<0 vs x>0) near‑wall φ overlays
  • Turning‑time estimates near the boundary, separated by side

It validates user choices; aggregates sessions at 7/14/21 dpf (or 70/140/210 for sanded); computes radial/tangential
components; and summarizes mean turning time by age. Adjustable parameters live in the CONFIG block below. Plot‑styling
numbers (e.g., y‑axis caps) intentionally remain inline for readability.
"""
...
# ==========================
RADIUS = 5.0                 # cm, arena radius for physical scaling and bounds
...
ANGLE_STEP = 0.01            # radians, sampling step for boundary angles in reflection check
...
Y_FLIP = -1                  # multiply y by this to flip image coords to math coords

# Phi histogram bin edges (0 to pi/2, open upper edge)
PHI_BIN_EDGES = np.linspace(0, np.nextafter(np.pi/2, 0), 9)

# ---------- User inputs & validation ----------
val = input('Sanded or Clear? : ').strip().lower()
if val == 'sanded':
    arr = [70, 140, 210]
elif val == 'clear':
    arr = [7, 14, 21]
else:
    print("Invalid input. Please enter 'Sanded' or 'Clear'. Exiting.")
    sys.exit(1)
...
def processtr(tr):
    """Center, set time/length units, and print diagnostics."""
    center, _ = tr.estimate_center_and_radius_from_locations(in_px=True)
    tr.origin_to(center)
    tr.new_length_unit(tr.params['body_length_px'], 'BL')
    tr.new_time_unit(tr.params['frame_rate'], 's')
    print('Positions:')
    print('X range:', np.nanmin(tr.s[..., 0]), np.nanmax(tr.s[..., 0]), 'BL')
    print('Y range:', np.nanmin(tr.s[..., 1]), np.nanmax(tr.s[..., 1]), 'BL')
    print('Velocities:')  # fixed label typo
    print('X range:', np.nanmin(tr.v[..., 0]), np.nanmax(tr.v[..., 0]), 'BL/s')
    print('Y range:', np.nanmin(tr.v[..., 1]), np.nanmax(tr.v[..., 1]), 'BL/s')
    print('Accelerations:')
    print('X range:', np.nanmin(tr.a[..., 0]), np.nanmax(tr.a[..., 0]), 'BL/s^2')
...
def plotReflection(xposition, yposition, xvelocity, yvelocity, radius):
    """Estimate fraction of boundary points visible as reflection, given position and heading."""
    mag = np.sqrt(xposition**2 + yposition**2)
    magv = np.sqrt(xvelocity**2 + yvelocity**2)
    _ = 2 * (radius - mag)  # geometric context (not used directly below)
...
def border_turning(tr, radius, refl_prop):
    """Convert to physical units, flip Y by convention, and accumulate reflection proportion per frame."""
    ...
    i = 0
    while i < len(pos1):
        prop = plotReflection(pos1[i][0], pos1[i][1], v1[i][0], v1[i][1], radius)
        refl_prop.append(prop)
        i += 1
...
for x in arr:
    ...
    for temp in trs:
        processed_temp = processtr(temp)
        border_turning(processed_temp, radius, refl_prop)
    ...
    # ----- Aggregate to arrays & basic heatmap -----
    phalf = np.concatenate(processedpos, axis=0)
    phalf = np.reshape(phalf, [phalf.shape[0] * phalf.shape[1], 2])
    ...
    # Fixed parenthesis bug for φ; then fold into [0, π/2]
    phi_temp = np.arccos(( -np.cos(half_df['theta'])*half_df['vx'] - np.sin(half_df['theta'])*half_df['vy'] )/half_df['spd'])
    ...
    if indiv == 'Y':
        ...
        plt.figure(figsize=(10, 6))
        sns.scatterplot(data=half_df, x='r', y='spd', hue='side', palette={'clear': 'blue', 'sanded': 'red'}, s=5, alpha=0.5)
        plt.title(f'Relationship between Speed and Radial Position Half Sanded {x}dpf')
...
plt.errorbar(x=arr, y=tt_avg_s, yerr=tt_std_s, fmt='o', color='red',  alpha=0.5, label='sanded')
plt.errorbar(x=arr, y=tt_avg_c, yerr=tt_std_c, fmt='o', color='blue', alpha=0.5, label='clear')
plt.legend(); plt.xlabel('Days Post Fertilization'); plt.ylabel('Turning Time Along Wall')
plt.title('Mean Turning Time over dpf (Half‑Sanded)')
plt.show()

# Tag each per-age frame, combine (avoid duplicate indices)
for i in range(len(outputs)):
    outputs[i]['Age'] = f"{arr[i]}dpf"
combined_df = pd.concat(outputs, ignore_index=True)

# ----- Combined r histogram across ages -----
sns.histplot(
...
for i, output in enumerate(outputs):
    ...
    sns.histplot(
        x=data, bins=PHI_BIN_EDGES, stat='percent', element='step', fill=False,
        linewidth=2.5, alpha=0.4, color=colors[i % len(colors)],
        label=(f"{int(arr[i]/10)}dpf (x<0)" if arr[i] % 10 == 0 else f"{arr[i]}dpf (x<0)"),
        common_norm=False,
    )
