...
# Histogram plot for control data
plt.figure(figsize=(5, 4))
plt.hist(data7, bins=30, range = [0,3],alpha=0.4, color='blue', label='7 dpf', weights=np.ones_like(data7) / len(data7) * 100, histtype='step', linewidth=5)
plt.hist(data14, bins=30, range = [0,3],alpha=0.4, color='purple', label='14 dpf', weights=np.ones_like(data14) / len(data14) * 100, histtype='step', linewidth=5)
plt.hist(data21, bins=30, range = [0,3],alpha=0.4, color='red', label='21 dpf', weights=np.ones_like(data21) / len(data21) * 100, histtype='step', linewidth=5)
plt.xlabel('Speed (cm/s)')
plt.ylabel('Percentage (%)')
plt.legend(title='Control')
plt.tight_layout()
plt.savefig("/Users/ezhu/Downloads/speed_histogram_control.png", dpi=3000, bbox_inches='tight')
plt.ylim(0,30)
...
# Histogram plot for blind data
plt.figure(figsize=(5, 4))
plt.hist(data7b, bins=30, range = [0,3],alpha=0.4, color='blue', label='7 dpf', weights=np.ones_like(data7b) / len(data7b) * 100, histtype='step', linewidth=5)
plt.hist(data14b, bins=30, range = [0,3],alpha=0.4, color='purple', label='14 dpf', weights=np.ones_like(data14b) / len(data14b) * 100, histtype='step', linewidth=5)
plt.hist(data21b, bins=30, range = [0,3],alpha=0.4, color='red', label='21 dpf', weights=np.ones_like(data21b) / len(data21b) * 100, histtype='step', linewidth=5)
plt.xlabel('Speed (cm/s)')
plt.ylabel('Percentage (%)')
plt.legend(title='Blind')
plt.tight_layout()
plt.savefig("/Users/ezhu/Downloads/speed_histogram_blind.png", dpi=3000, bbox_inches='tight')
plt.yticks(np.arange(0, 31, 10))
plt.ylim(0,30)
plt.xlim(0, 3)
plt.show()